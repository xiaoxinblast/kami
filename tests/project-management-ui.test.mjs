...
test("项目管理提供新建向导、删除入口和稳定的表单重置", async () => {
  ...
  assert.match(html, /id="deleteProjectList"/u);
  assert.match(html, /id="deleteProjectSelectAll"/u);
  assert.match(html, /id="deleteProjectCount"/u);
  assert.match(html, /id="deleteProjectConfirm"/u);
